/**
 * Component Job5DisplayareaComponent is defined as
 * `<e-job5-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job5DisplayareaComponent();
 *
...
import style from './job5DisplayareaComponent.css';

/**
 * @property {Boolean} propOne - show active/inactive state.
 * @property {String} propTwo - shows the "Hello World" string.
 */
@definition('e-job5-displayarea-component', {
  style,
  home: 'job5-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job5DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/130/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/130/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/130')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/130')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
...
/**
 * Register the component as e-job5-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job5DisplayareaComponent.register();
