/**
 * Component Job3DisplayareaComponent is defined as
 * `<e-job3-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job3DisplayareaComponent();
 *
 * Declaratively create component
 * @example
 * <e-job3-displayarea-component></e-job3-displayarea-component>
 *
 * @extends {LitComponent}
 */
 import { definition } from '@eui/component';
 import { LitComponent, html } from '@eui/lit-component';
 import style from './job3DisplayareaComponent.css';
 import '@eui/table';
 import '@eui/layout';
 import '@eui/base';
  ...
  */
 @definition('e-job3-displayarea-component', {
   style,
   home: 'job3-displayarea-component',
   props: {
     propOne: { attribute: true, type: Boolean },
     propTwo: { attribute: true, type: String, default: 'Hello World' },
   },
 })
 export default class Job3DisplayareaComponent extends LitComponent {
   ...
   async _makeData(){
     ...
     while (localloop < localto) {
       ...
       let answer = new Promise((res, rej) => {
         fetch('http://localhost:8080/build-recovery-time-v2/job/128/time-period', {
           method: 'POST',
           body: jsonobj,
       ...
       answer = new Promise((res, rej) => {
         fetch('http://localhost:8080/build-failure-rate/job/128/time-period', {
           method: 'POST',
           body: jsonobj,
           headers: { 'Content-Type': 'application/json' }
         }).then(res => res.json())
   ...
   async loadNames() {
     fetch('http://localhost:8080/build-failure-rate/job/128')
   ...
   async loadStats(){
     fetch('http://localhost:8080/build-recovery-time-v2/job/128')
       .then(function (response) {
         return response.json();
       })
       .then(function (data) {
 ...
 Job3DisplayareaComponent.register();
