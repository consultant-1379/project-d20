/**
 * Component Job7DisplayareaComponent is defined as
 * `<e-job7-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job7DisplayareaComponent();
 *
 * Declaratively create component
 * @example
 * <e-job7-displayarea-component></e-job7-displayarea-component>
 *
 * @extends {LitComponent}
 */
import { definition } from '@eui/component';
import { LitComponent, html } from '@eui/lit-component';
import style from './job7DisplayareaComponent.css';
import '@eui/table';
import '@eui/layout';
import '@eui/base';
 ...
 */
@definition('e-job7-displayarea-component', {
  style,
  home: 'job7-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job7DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/132/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/132/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/132')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/132')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
...
/**
 * Register the component as e-job7-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job7DisplayareaComponent.register();
