/**
 * Component Job6DisplayareaComponent is defined as
 * `<e-job6-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job6DisplayareaComponent();
 *
 * Declaratively create component
 * @example
 * <e-job6-displayarea-component></e-job6-displayarea-component>
 *
 * @extends {LitComponent}
 */
import { definition } from '@eui/component';
import { LitComponent, html } from '@eui/lit-component';
import style from './job6DisplayareaComponent.css';

/**
 * @property {Boolean} propOne - show active/inactive state.
 * @property {String} propTwo - shows the "Hello World" string.
 */
@definition('e-job6-displayarea-component', {
  style,
  home: 'job6-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job6DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/131/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/131/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/131')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/131')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
...
/**
 * Register the component as e-job6-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job6DisplayareaComponent.register();
