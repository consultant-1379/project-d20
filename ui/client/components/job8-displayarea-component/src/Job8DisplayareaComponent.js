/**
 * Component Job8DisplayareaComponent is defined as
 * `<e-job8-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job8DisplayareaComponent();
 *
...
import style from './job8DisplayareaComponent.css';
import '@eui/table';
import '@eui/layout';
import '@eui/base';
 ...
 */
@definition('e-job8-displayarea-component', {
  style,
  home: 'job8-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job8DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/133/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/133/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/133')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/133')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
...
/**
 * Register the component as e-job8-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job8DisplayareaComponent.register();
