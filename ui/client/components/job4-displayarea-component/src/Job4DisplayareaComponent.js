/**
 * Component Job4DisplayareaComponent is defined as
 * `<e-job4-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job4DisplayareaComponent();
 *
 * Declaratively create component
 * @example
 * <e-job4-displayarea-component></e-job4-displayarea-component>
 *
 * @extends {LitComponent}
 */
import { definition } from '@eui/component';
import { LitComponent, html } from '@eui/lit-component';
import style from './job4DisplayareaComponent.css';
import '@eui/table';
import '@eui/layout';
import '@eui/base';
 ...
 */
@definition('e-job4-displayarea-component', {
  style,
  home: 'job4-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job4DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/129/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/129/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/129')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/129')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
        this.unstableBuildsData.push({ col1: 'Average Build Failure Recovery Time', col2: data.medianInMinutes, colorBand: 'red' },
...
/**
 * Register the component as e-job4-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job4DisplayareaComponent.register();
