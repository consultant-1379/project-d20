/**
 * Component Job9DisplayareaComponent is defined as
 * `<e-job9-displayarea-component>`
 *
 * Imperatively create component
 * @example
 * let component = new Job9DisplayareaComponent();
 *
 * Declaratively create component
 * @example
 * <e-job9-displayarea-component></e-job9-displayarea-component>
 *
 * @extends {LitComponent}
 */
import { definition } from '@eui/component';
import { LitComponent, html } from '@eui/lit-component';
import style from './job9DisplayareaComponent.css';
import '@eui/table';
import '@eui/layout';
import '@eui/base';
 ...
 */
@definition('e-job9-displayarea-component', {
  style,
  home: 'job9-displayarea-component',
  props: {
    propOne: { attribute: true, type: Boolean },
    propTwo: { attribute: true, type: String, default: 'Hello World' },
  },
})
export default class Job9DisplayareaComponent extends LitComponent {
  doGraph(jsonData) {
    const json = jsonData;
  ...
  async _makeData(){
    ...
    while (localloop < localto) {
      ...
      let answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-recovery-time-v2/job/134/time-period', {
          method: 'POST',
          body: jsonobj,
      ...
      answer = new Promise((res, rej) => {
        fetch('http://localhost:8080/build-failure-rate/job/134/time-period', {
          method: 'POST',
          body: jsonobj,
          headers: { 'Content-Type': 'application/json' }
        }).then(res => res.json())
  ...
  async loadNames() {
    fetch('http://localhost:8080/build-failure-rate/job/134')
  ...
  async loadStats(){
    fetch('http://localhost:8080/build-recovery-time-v2/job/134')
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
...
/**
 * Register the component as e-job9-displayarea-component.
 * Registration can be done at a later time and with a different name
 */
Job9DisplayareaComponent.register();
